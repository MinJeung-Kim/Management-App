...
import { pay } from "../../locale/constant";

const CardAddForm = memo(({ FileInput, onAdd, sale, setSale, openEditor }) => {
  ...
  const onDiscountChange = (value) => {
    value === "할인" ? setSale(pay.SALE) : setSale(0);
  };
  ...
  const onSubmit = (event) => {
    ...
    const { name, age, job, discount, period, registration, phone } = formItems;
    const card = {
      id: Date.now(), //uuid
      name: name,
      gender: gender,
      ...
      <form ref={formRef} className={styles.form}>
        ...
        <div className={styles.pricegroup}>
          <div className={styles.pricebox}>
            <p className={styles.price}>결제 금액</p>
            <p>{new Intl.NumberFormat().format(pay.PAY_PRICE)}</p>
          </div>
          <div className={styles.pricebox}>
            <p className={styles.price}>할인 금액</p>
            <p>{new Intl.NumberFormat().format(sale)}</p>
          </div>
          <hr />
          <div className={styles.pricebox}>
